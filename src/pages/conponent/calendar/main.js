...
const conf = {
	...
	calculateDays(year, month, curDate) {
		let days = [];
		var curday = new Date().getUTCDate();
		var curyear = new Date().getUTCFullYear();
		var curmonth = new Date().getUTCMonth() + 1;
		this.setData({
			"calendar.curday": curday,
			"calendar.curyear": curyear,
			"calendar.curmonth": curmonth,
			"calendar.today": curyear + "," + curmonth + "," + curday
		});
		...
		let today = Date.UTC(
			this.data.calendar.curyear,
			this.data.calendar.curmonth,
			this.data.calendar.curday
		);
		days.forEach(item => {
			const cur = Date.UTC(item.year, item.month, item.day);
		...
		this.setData(tmp);
		console.log(this.data.calendar.days, this.data.curday);
	},
	/**
	 * 当改变月份时触发
	 * @param {object} param
...
export const jump = (year, month, day) => {
	...
	if (
		selectedDay &&
		selectedDay[0] + selectedDay[0].year === +year &&
